...
class GiaiTichApp(QDialog):
    def __init__(self):
        ...
        # Nút vẽ đồ thị
        self.plot_button = QPushButton("Vẽ đồ thị")
        ...
        button_layout.addWidget(self.clear_button)  # Thêm nút xóa dữ liệu
        button_layout.addWidget(self.plot_button)  # Thêm nút vẽ đồ thị
    ...
    def plot_graph(self):
        ...
        try:
            ...
            # Tính toán giá trị y cho x trong khoảng
            x_vals = np.linspace(-10, 10, 400)
            y_vals = [poly.subs(symbols('x'), val) for val in x_vals]

            # Vẽ đồ thị
            plt.figure()
            plt.plot(x_vals, y_vals, label=str(poly))
            plt.title("Đồ thị của đa thức")
            plt.xlabel("x")
            plt.ylabel("y")
            ...
            QMessageBox.warning(self, "Lỗi", f"Không thể vẽ đồ thị: {e}")

    def clear_data(self):
        """Xóa dữ liệu trong bảng, các ô nhập và nhãn kết quả."""
        # Xóa hệ số trong bảng
        self.table_widget.clearContents()
        
        # Xóa ô nhập cận trên và cận dưới
        self.lower_limit_input.clear()
        self.upper_limit_input.clear()
        
        # Đặt lại nhãn kết quả
        self.result_label.setText("Kết quả: ")
        
        # Đặt lại lựa chọn bài toán
